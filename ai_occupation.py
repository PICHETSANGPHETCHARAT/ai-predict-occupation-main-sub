...
import os
import re
from api_usage_logger import api_logger, EndpointType
...
# =============================================================================
# CLOUDFLARE 524 MITIGATION — HEARTBEAT STREAMING HELPER
# =============================================================================
async def stream_with_heartbeat(work_coro, build_payload, on_error=None, interval=15.0):
...
async def predict_member_api(data: PredictRequestMember):
    """API ผู้หางาน ทำนายตำแหน่งงาน (heartbeat streaming กัน Cloudflare 524)"""
    ...
    async def do_work():
        # AsyncOpenAI ปล่อย event loop ระหว่าง 2 GPT calls → heartbeat yield ได้
        result = await handle_prediction_gpt(None, job_title, render_html=False)
        if result.get("error"):
            # GPT มีปัญหา fallback ThaiBERT (เรียก handle_prediction_gpt อีกรอบ — เผื่อ retry)
            print(f"GPT prediction failed: {result.get('error')}, falling back to ThaiBERT")
            result = await handle_prediction_gpt(None, job_title, render_html=False)
        return result

    def build_success(result):
        api_logger.log_fire_and_forget(
            EndpointType.PREDICT_MEMBER,
            processing_time_ms=(time_module.time() - start_time) * 1000,
            success=True,
            extra_data={"job_title": job_title},
        )
        return result
    ...
    def build_error(exc):
        ...
        return {"success": False, "status_code": 500, "error": error_msg}

    return StreamingResponse(
        stream_with_heartbeat(do_work(), build_success, on_error=build_error, interval=15.0),
...
async def predict_employer_api(data: PredictRequestEmployer):
    """API นายจ้าง ทำนายตำแหน่งงาน+ประเภทธุรกิจ (heartbeat streaming)"""
    job_title = data.job_title
    ...
    bussiness_type = bussiness_type.strip()
    bussiness_type = re.sub(r'[^\w\sก-๙]', '', bussiness_type)
    ...
    async def do_work():
        ...
        if result.get("error"):
            print(f"GPT prediction with business type failed: {result.get('error')}, falling back to ThaiBERT")
            result = await handle_prediction_gpt(None, job_title, render_html=False)
        return result

    def build_success(result):
        api_logger.log_fire_and_forget(
            EndpointType.PREDICT_EMPLOYER,
            processing_time_ms=(time_module.time() - start_time) * 1000,
            success=True,
            extra_data={"job_title": job_title, "bussiness_type": bussiness_type},
        )
        return result
